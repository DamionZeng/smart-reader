...
function toConceptGraph(row: typeof conceptGraphs.$inferSelect): ConceptGraph {
  ...
  // Strip HTML for enrichment: enrichConcepts sends rawText to LLM as
  // source context, and anchors must be verbatim plain text sentences.
  const rawHtml = row.rawText || "";
  const plainRawText = isHtml(rawHtml) ? stripHtml(rawHtml) : rawHtml;

  return {
    id: row.id,
    title: row.title,
    type: row.type as "paper" | "code",
    rawText: plainRawText,
...
// POST /api/concept-graph/[id]/enrich — enrich concepts with LLM descriptions
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    let result;
    try {
      result = await enrichConcepts(graph, langInstruction);
    } catch (e: any) {
    ...
    await trackUsage(session.user.id, "concept-graph-enrich");

    return NextResponse.json({ graph: toConceptGraph(updated) });
  } catch (error: any) {
    console.error("Enrich concept graph error:", error);
    return NextResponse.json(
